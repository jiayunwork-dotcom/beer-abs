...
go test ./...        # 全部单元测试（law / band / mixture / calib / spectrum / server）
```
...
curl -s -X POST localhost:8080/api/absorbance -H 'Content-Type: application/json' \
  -d '{"components":[{"extinction":125,"concentration":0.008}],"path_length":1}'
```
